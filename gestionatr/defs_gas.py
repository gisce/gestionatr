...
TAULA_MOTIVOS_DE_RECHAZO_CNMC = [
    ...
    ('R03', 'El CUPS no está conectado al distribuidor que se indica en la solicitud.'),
    ('R04', 'Falta infraestructura.'),
    ('R07', 'Si el modelo fecha efecto que solicita es "03" o “04” debe cumplimentar el campo "Fecha efecto solicitada" '),
    ('R08', 'La fecha de efecto solicitada "@replace1@" no puede ser mayor que la fecha "@replace2@".'),
    ('R11', 'El municipio "@replace1@" no pertenece a la provincia "@replace2@".'),
    ('R12', 'Error de integridad: DNI no coherente con CUPS. '),
    ('R14', 'El día de la fecha efecto solicitada es errónea, tiene que ser el 1, 11 ó 21 del mes. '),
    ('R19', 'Si el motivo de modificación es @replace1@, los campos @replace2@ tienen que ser distintos.'),
    ('R24', 'El nuevo peaje no puede coincidir con el peaje actual del contrato.'),
    ('R26', 'Nuevo Caudal Diario no puede coincidir con el Caudal Diario Actual del contrato.'),
    ('R29', 'CUPS inexistente.'),
    ('R32', 'Fecha efecto solicitada anterior al día actual.'),
    ('R34', 'No existe contrato de ATR en vigor.'),
    ('R35', 'Existe una solicitud previa del tipo nuevo suministro del mismo comercializador.'),
    ('R36', 'Existe una solicitud previa del tipo baja del mismo comercializador.'),
    ('R37', 'Existe una solicitud previa de tipo cambio del mismo comercializador.'),
    ('R38', 'Existe una solicitud previa de tipo modificación del mismo comercializador.'),
    ('R39', 'Existe una solicitud previa de tipo anulación del mismo comercializador.'),
    ('R40', 'El comercializador no es la empresa titular del contrato ATR en vigor.'),
    ('R41', 'No existe una previa del comercializador que solicita la Anulación.'),
    ('R42', 'La solicitud que se intenta anular ya está "en trámite" o formalizada.'),
    ('R43', 'El CUPS ya está activo con la comercializadora solicitante.'),
    ('R44', 'Existe una solicitud previa del tipo alta de otra Comercializadora.'),
    ...
    ('R49', 'Suministro a Presión superior a 16 bares. Debe presentarse la solicitud a través del transportista.'),
    ('R51', 'CUPS en servicio con otra comercializadora.'),
    ('R61', 'Si el motivo de modificación es @replace1@, se debe enviar alguno de los campos @replace2@.'),
    ('R62', 'Si la presión solicitada es mayor de 4 bares o el peaje es 3.5, es obligatorio que el campo Caudal Diario tenga un valor'),
    ('R64', 'El número de documento informado no cumple con las validaciones definidas sobre los documentos del tipo enviado'),
    ('R70', 'Otros motivos no contemplados en el resto del listado.'),
    ('R73', 'No se cumple período mínimo desde la última modificación de peaje o Qd'),
    ...
    ('R78', 'El Código de Solicitud ya ha sido recibido con anterioridad para ese mismo agente'),
    ('R79', 'Existe una Solicitud de Actuación (corte del suministro) en curso del mismo comercializador'),
    ('R80', 'Existe una Solicitud de Actuación (corte del suministro) en curso de otro comercializador'),
    ...
    ('R94', 'No es posible solicitar un cambio/alta/modificación a fecha fija para una fecha > 30 días naturales'),
    ('R95', 'Existe una solicitud previa de baja ya en campo o realizada (pendiente de activar) de otra comercializadora.'),
